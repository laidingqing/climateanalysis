...
/**
 * Custom version of HFileOutputFormat2 which cherry picks from the result of HBASE-12596.
...
@Public
@Evolving
public class CustomHFileOutputFormat2 extends FileOutputFormat<ImmutableBytesWritable, Cell> {
    static Log LOG = LogFactory.getLog(CustomHFileOutputFormat2.class);
    ...
    public CustomHFileOutputFormat2() {
    }
    ...
    static <V extends Cell> RecordWriter<ImmutableBytesWritable, V> createRecordWriter(final TaskAttemptContext context) throws IOException {
        ...
        return new RecordWriter<ImmutableBytesWritable, V>() {
            private final Map<byte[], CustomHFileOutputFormat2.WriterLength> writers;
            ...
            public void write(ImmutableBytesWritable row, V cell) throws IOException {
                ...
                if(row == null && kv == null) {
                    ...
                    CustomHFileOutputFormat2.WriterLength wl = (CustomHFileOutputFormat2.WriterLength)this.writers.get(family);
                    if(wl == null) {
            ...
            private void rollWriters() throws IOException {
                CustomHFileOutputFormat2.WriterLength wl;
                for(Iterator i$ = this.writers.values().iterator(); i$.hasNext(); wl.written = 0L) {
                    wl = (CustomHFileOutputFormat2.WriterLength)i$.next();
                    if(wl.writer != null) {
                        CustomHFileOutputFormat2.LOG.info("Writer=" + wl.writer.getPath() + (wl.written == 0L?"": ", wrote=" + wl.written));
            ...
            private CustomHFileOutputFormat2.WriterLength getNewWriter(byte[] family, Configuration confx, InetSocketAddress[] favoredNodes) throws IOException {
                CustomHFileOutputFormat2.WriterLength wl = new CustomHFileOutputFormat2.WriterLength();
            ...
            public void close(TaskAttemptContext c) throws IOException, InterruptedException {
                ...
                while(i$.hasNext()) {
                    CustomHFileOutputFormat2.WriterLength wl = (CustomHFileOutputFormat2.WriterLength)i$.next();
                    this.close(wl.writer);
                }
    ...
    public static void configureIncrementalLoad(Job job, HTable table) throws IOException {
        configureIncrementalLoad(job, table, table.getRegionLocator());
    }

    public static void configureIncrementalLoad(Job job, Table table, RegionLocator regionLocator) throws IOException {
        configureIncrementalLoad(job, table, regionLocator, CustomHFileOutputFormat2.class);
    }
    ...
    public static void configureIncrementalLoadMap(Job job, Table table) throws IOException {
        ...
        job.setOutputKeyClass(ImmutableBytesWritable.class);
        job.setOutputValueClass(KeyValue.class);
        job.setOutputFormatClass(CustomHFileOutputFormat2.class);
        configureCompression(table, conf);
        configureBloomType(table, conf);
        configureBlockSize(table, conf);
        configureDataBlockEncoding(table, conf);
